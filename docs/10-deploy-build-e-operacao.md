...
3. se `APP_PROCESS=realtime`: sobe os pollers em background (`run_due_outbound_messages` a cada `OUTBOUND_POLLER_INTERVAL_SECONDS`, default 60s, e `sync_sefaz_documents` a cada `SEFAZ_SYNC_INTERVAL_SECONDS`, default 300s) e o Daphne (`config.asgi:application`) na porta `PORT`
