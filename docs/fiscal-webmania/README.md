...
Fase atual: Fase 3.10.0 - Reavaliacao da Manifestacao da NFS-e Manual, em planejamento documental em 2026-06-29. A Fase 3.9.1 foi validada e encerrada no checkpoint `99254f33`, implementando a substituicao da NFS-e manual por extensao segura de `NfseSubstitutionPreview`, `NfseSubstitution` e `operation_type="nfse_substitution"`, com payload remoto em `POST /2/nfse/substituir`, XML original preservado e nova `NfseItem` substituta criada somente apos confirmacao remota valida. Manifestacao da NFS-e manual continua pendente e deve ser reavaliada antes de qualquer codigo funcional.
...
| 3.10.0 - Reavaliacao manifestacao NFS-e manual | em planejamento | `03-prd-matriz-api-webmania.md`, `07-plano-de-fases-e-criterios-de-aceite.md`, `09-decisoes-arquiteturais.md` | N/A | N/A | Somente documental; decidir entre manifestacao manual, adiamento ou NFS-e recebida/importada |
