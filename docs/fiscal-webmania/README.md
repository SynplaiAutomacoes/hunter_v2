...
Fase atual: Fase 3.8.0 - Reavaliacao do Ciclo Pos-Emissao Manual de NFS-e, em planejamento documental em 2026-06-28. A Fase 3.7.1 foi validada e encerrada no checkpoint `2cb35206`, com emissao manual nova de NFS-e implementada exclusivamente a partir de `NfseManualEmissionPreview` aprovada. A implementacao criou `NfseManualEmission`, `FiscalEmissionAttempt(operation_type="nfse_manual_emission")`, `NfseItem` somente apos confirmacao remota valida, e webhook/reconciliacao consultivos sem repetir `POST`.
...
| 3.8.0 - Reavaliacao pos-emissao manual NFS-e | em planejamento | `03-prd-matriz-api-webmania.md`, `07-plano-de-fases-e-criterios-de-aceite.md`, `09-decisoes-arquiteturais.md` | N/A | N/A | Somente documentacao; escolher proximo ciclo da NFS-e manual |
