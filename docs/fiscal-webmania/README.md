...
Fase atual: Fase 3.16.1 - Ampliacao Operacional da Inbox XML NFS-e, em implementacao tecnica em 2026-06-30. A Fase 3.16.0 foi validada documentalmente e commitada no checkpoint `267fc601`, com decisao aprovada de ampliar a inbox local antes de qualquer conector real. A 3.16.1 mantem o fluxo inbox -> revisao humana -> lote XML -> `NfseReceivedDocument`, adicionando filtros/busca, relatorio CSV sem XML bruto, acoes em massa controladas, permissoes especificas e auditoria por usuario/data ja persistida nos itens. Permanecem fora de escopo conectores reais de e-mail/ERP, IMAP/Gmail/Microsoft Graph/OAuth, pasta monitorada/Drive/SharePoint, webhook externo, job agendado, consulta Webmania automatica, manifestacao automatica, documento recebido sem XML, `NfseItem`, `FiscalDocument(nfse)` e `FiscalEmissionAttempt`.
...
| 3.16.1 - Ampliacao operacional da inbox XML NFS-e | em implementacao | `04-prd-dominio-e-modelagem.md`, `05-prd-seguranca-idempotencia-webhooks.md`, `06-prd-ux-permissoes-e-workflows.md`, `08-plano-de-testes-e-operacao.md` | 2026-06-30 | N/A | Filtros/busca, CSV sem XML bruto, acoes em massa e permissoes; sem conectores reais/Webmania automatica |
