...
Fase atual: Fase 3.9.0 - Reavaliacao apos Ciclo Minimo da NFS-e Manual, em planejamento documental em 2026-06-28. A Fase 3.8.1 foi validada e encerrada no checkpoint `29f3f3a9`, concluindo o ciclo minimo `preview -> emissao -> cancelamento` da NFS-e manual nova. O cancelamento manual reutiliza `NfseCancellation` e `nfse_cancellation`; `NfseCancellation.request` agora e opcional para origem manual; o contrato remoto permanece `PUT /2/nfse/cancelar` com `{uuid, motivo}`; o XML original da emissao e preservado e o XML de cancelamento fica separado.
...
| 3.9.0 - Reavaliacao apos ciclo minimo NFS-e manual | em planejamento | `03-prd-matriz-api-webmania.md`, `07-plano-de-fases-e-criterios-de-aceite.md`, `09-decisoes-arquiteturais.md` | N/A | N/A | Somente documental; escolher proximo bloco apos preview/emissao/cancelamento |
