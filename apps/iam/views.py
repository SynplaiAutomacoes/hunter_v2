...
class WorkshopRoleListView(WorkshopScopedMixin, HtmxTemplateResponseMixin, ListView):
    ...
    def get_queryset(self):
        return ListView.get_queryset(self).filter(account=self.request.user.account).order_by("-criado_em")
...
class WorkshopRoleUpdateView(DirectorOnlyDeliveryDatePermissionMixin, WorkshopScopedMixin, UpdateView):
    ...
    def get_queryset(self):
        return UpdateView.get_queryset(self).filter(account=self.request.user.account)
...
class WorkshopRoleDeleteView(WorkshopScopedMixin, HtmxDeleteResponseMixin, DeleteView):
    model = WorkshopRole
    success_url = reverse_lazy("iam:role_list")
    ...
    def get_queryset(self):
        return DeleteView.get_queryset(self).filter(account=self.request.user.account)
