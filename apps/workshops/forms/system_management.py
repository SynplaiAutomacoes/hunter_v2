...
class NotificationBroadcastForm(forms.Form):
    ...
    user_select = forms.ChoiceField(
        ...
        widget=SearchableSelectInput(choices=[]),
        help_text="Pesquise e selecione usuários com permissão/login para adicionar à lista de destinatários.",
    )
    ...
    workshop_targets_data = forms.CharField(
        required=False,
    ...
    message = forms.CharField(
        ...
        widget=forms.Textarea(attrs={"class": "textarea textarea-bordered w-full", "rows": 4, "placeholder": "Digite a mensagem que será enviada aos destinatários..."}),
    )
