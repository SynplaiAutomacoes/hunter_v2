...
class CollaboratorPayrollServiceTests(TestCase):
    ...
    def test_sync_workorder_collaborator_payrolls_creates_forecast_commission_from_services_total(self) -> None:
        ...
        product_group = CatalogGroup.objects.create(workshop=workshop, name="Grupo Produto Comissao")
        product = Product.objects.create(
            workshop=workshop,
            group=product_group,
            code="COM-01",
            name="Produto Comissao",
    ...
    def test_recalculate_commission_history_repairs_existing_entries_and_payrolls(self) -> None:
        workshop = create_workshop(suffix=9)
        collaborator = create_collaborator(workshop=workshop, suffix=9, receives_commission=True)
        WorkshopCost.objects.create(workshop=workshop, month=5, year=2026, mechanic_quantity=1, work_days_per_month=20)
        budget = create_budget(workshop=workshop)
        budget.status = "approved"
        budget.save(update_fields=["status"])
        workorder = WorkOrder.objects.create(workshop=workshop, budget=budget, status=WorkOrderStatus.APPROVED)
        workorder.collaborators.add(collaborator)
        product_group = CatalogGroup.objects.create(workshop=workshop, name="Grupo Produto Historico")
        product = Product.objects.create(
            workshop=workshop,
            group=product_group,
            code="HIS-01",
        ...
        service = Service.objects.create(workshop=workshop, name="Servico Historico", duration=timedelta(hours=1), suggested_cost=Money("50.00", "BRL"), selling_price=Money("200.00", "BRL"))
        WorkOrderItem.objects.create(workshop=workshop, workorder=workorder, product=product, quantity=1)
        WorkOrderItem.objects.create(workshop=workshop, workorder=workorder, service=service, quantity=1)
        WorkOrderPaymentMethod.objects.create(workorder=workorder, due_date=date(2026, 5, 20), first_installment_amount=Money("300.00", "BRL"), remaining_installments_amount=Money("0.00", "BRL"), installments_count=1)
        ...
        payroll = sync_workorder_collaborator_payrolls(workorder=workorder, reference_date=date(2026, 5, 1))[0]
        entry = CollaboratorCommissionEntry.objects.get(collaborator=collaborator, workorder=workorder)
        commission_item = CollaboratorPayrollItem.objects.get(payroll=payroll, item_type=CollaboratorPayrollItem.ItemType.COMMISSION)

        entry.base_amount = Money("300.00", "BRL")
        entry.commission_amount = Money("30.00", "BRL")
        entry.save(update_fields=["base_amount", "commission_amount"])

        payroll.commission_amount = Money("30.00", "BRL")
        payroll.total_amount = Money("1110.00", "BRL")
        payroll.save(update_fields=["commission_amount", "total_amount"])
        assert payroll.financial_movement is not None
        payroll.financial_movement.amount = Money("1110.00", "BRL")
        payroll.financial_movement.save(update_fields=["amount"])

        commission_item.amount = Money("30.00", "BRL")
        commission_item.description = "10.00% sobre R$ 300,00"
        commission_item.save(update_fields=["amount", "description"])
        ...
        self.assertIn("1 comissões e 1 folhas atualizadas", stdout.getvalue())
        self.assertEqual(entry.base_amount, Money("200.00", "BRL"))
        self.assertEqual(entry.commission_amount, Money("20.00", "BRL"))
        self.assertEqual(payroll.commission_amount, Money("20.00", "BRL"))
        self.assertEqual(payroll.total_amount, Money("1100.00", "BRL"))
        self.assertEqual(commission_item.amount, Money("20.00", "BRL"))
