...
class Budget(TimeStampedModel):
    ...
    def signature_blockers(self) -> list[str]:
        blockers: list[str] = []
        if not self.service_expected_completion_at:
            blockers.append("Não é possível enviar para assinatura antes de definir a data prevista de término do serviço.")
        return blockers
    ...
    def get_total_products_by_slider_without_shipping(self):
        # Kept for compatibility with existing callers. Freight is no longer part of sale totals.
        return self.get_total_products_by_slider
...
class BudgetItem(TimeStampedModel):
    ...
    def summary_products_total_without_shipping(self) -> Money:
        if self.is_customer_supplied:
            return Money(0, "BRL")
        if self.kit:
            return self.get_kit_products_total()
