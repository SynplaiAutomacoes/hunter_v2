...
def _apply_authorized_purchase_return_stock(*, request_id: int) -> PurchaseReturnRequest:
    ...
    items = list(return_request.items.filter(source_item__isnull=False).select_related("source_item__stock_product", "source_item__stock_product__supplier").order_by("pk"))
    if not items:
