...
class ImportStepSummaryForm(CoreModelForm):
    ...
    def save(self, commit=True):
        ...
        for pay in payments_data:
            ...
            should_create_fm = False

            if financial_movement_id:
                if not FinancialMovement.objects.filter(pk=financial_movement_id, workshop=workshop).exists():
                    should_create_fm = True
            else:
                should_create_fm = True
            ...
            if should_create_fm:
                ...
                pay["financial_movement_id"] = fm.pk
