...
class TransportRequestWorkflowTests(TestCase):
    ...
    def test_five_step_workflow_renders_transport_data_without_creating_fiscal_document(self) -> None:
        ...
        self.assertContains(response, 'aria-labelledby="carrier-heading"')
        self.assertContains(response, 'aria-labelledby="vehicle-heading"')
        self.assertContains(response, 'aria-labelledby="volumes-heading"')
        self.assertContains(response, 'aria-labelledby="additional-information-heading"')
        self.assertContains(response, "Detalhes opcionais dos volumes")
        self.assertContains(response, "Reboques")
        self.assertContains(response, 'name="transport_vehicle_plate"', count=1)
        self.assertContains(response, 'name="transport_volume_brand"', count=1)
        self.assertContains(response, 'name="transport_trailer_3_ferry"', count=1)
        self.assertContains(response, 'name="additional_information"', count=1)
