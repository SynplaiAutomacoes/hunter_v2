...
class SharedEmissionWorkorderSelectionForm(CoreModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if selected_workorder is not None:
            has_nfe = NfeRequest.objects.filter(workorder=selected_workorder, soft_deleted_at__isnull=True).exists()
            has_nfse = NfseRequest.objects.filter(workorder=selected_workorder, soft_deleted_at__isnull=True).exists()
