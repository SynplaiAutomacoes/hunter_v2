...
class NfseSubstitutionPreviewCreateForm(CoreForm):
    ...
    environment = forms.ChoiceField(label="Ambiente", choices=(("1", "Producao"), ("2", "Homologação")), widget=SearchableSelectInput(choices=(("1", "Producao"), ("2", "Homologação"))))
    reason_code = forms.ChoiceField(label="Motivo", choices=((1, "Erro na emissão"), (2, "Serviço não prestado"), (4, "Duplicidade da nota")), widget=SearchableSelectInput(choices=((1, "Erro na emissão"), (2, "Serviço não prestado"), (4, "Duplicidade da nota"))))
    rps_number = forms.IntegerField(label="Número do novo RPS", min_value=1, widget=NumberInput(attrs={"min": "1"}))
