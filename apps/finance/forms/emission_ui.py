    ...
    *,
    ...
    layout_children.append(
        Div(
            Div(
                HTML(body_html),
                css_class="col-span-12 lg:col-span-8 space-y-6",
            ),
            Div(
                Div(
                    Div(
                ...
                css_class="col-span-12 lg:col-span-4 self-start",
            ),
...
def build_step5_preview_oob_html(*, prefix: str, panel_data: Step5PricingPanelData, warning_html: str, preview_html: str, include_sale_spans: bool = True) -> str:
    # include_sale_spans=False for layouts without the pricing panel, so htmx does not
    # try to swap spans that are absent from the DOM.
    sale_spans_html = ""
    if include_sale_spans:
        sale_spans_html = f"""
