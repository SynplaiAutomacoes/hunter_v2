...
def validate_correction_text(correction_text: str) -> str:
    ...
    if len(normalized) < CCE_MIN_TEXT_LENGTH or len(normalized) > CCE_MAX_TEXT_LENGTH:
        raise NfeCorrectionError("Informe uma correção entre 15 e 1000 caracteres.")
    ...
    if any(re.search(pattern, normalized_search_text) for pattern in CCE_PROHIBITED_TEXT_PATTERNS):
        raise NfeCorrectionError("A carta de correção não pode alterar valores, impostos, produtos, quantidades, destinatario, datas, serie, número ou outros dados fiscais essenciais da NF-e.")
    return normalized
...
def ensure_fiscal_document_for_nfe_item(*, item: NfeItem) -> FiscalDocument:
    if not is_nfe_item_eligible_for_cce(item):
        raise NfeCorrectionError("Carta de correção permitida somente para NF-e autorizada com chave de acesso ou UUID válido.")
...
def confirm_cce_event_from_payload(*, event: FiscalDocumentEvent, response_payload: dict[str, Any]) -> FiscalDocumentEvent:
    ...
    if event.status in {FiscalDocumentEventStatus.APPROVED, FiscalDocumentEventStatus.SUCCEEDED}:
        ...
        message = extract_webmania_error_message(response_payload, scope="nfe") or "Carta de correção rejeitada."
...
def _resolve_cce_remote_uuid(*, event: FiscalDocumentEvent) -> str:
    ...
    if not remote_uuid:
        raise NfeCorrectionError("Carta de correção em estado incerto sem UUID remoto. Aguarde o webhook antes de tentar novamente.")
    _parse_cce_uuid(remote_uuid, error_message="Carta de correção em estado incerto com UUID remoto inválido. Aguarde identificacao segura antes de reconciliar.")
    return remote_uuid
...
def validate_cce_payload_identity(
    *,
    ...
    if payload_model != "cce":
        raise NfeCorrectionError("A consulta retornou um documento diferente da carta de correção esperada.")

    if require_uuid and not payload_uuid:
        raise NfeCorrectionError("A resposta da carta de correção não possui UUID remoto seguro.")
    if payload_uuid:
        parsed_payload_uuid = _parse_cce_uuid(payload_uuid, error_message="A resposta da carta de correção possui UUID remoto inválido.")
        if expected_uuid and parsed_payload_uuid != _parse_cce_uuid(expected_uuid, error_message="A carta de correção local possui UUID remoto inválido."):
            raise NfeCorrectionError("A consulta retornou um documento diferente da carta de correção esperada.")

    payload_sequence = payload.get("evento")
    if require_sequence and payload_sequence in (None, ""):
        raise NfeCorrectionError("A resposta da carta de correção não possui sequencia segura.")
    if payload_sequence not in (None, "") and (not str(payload_sequence).isdigit() or int(payload_sequence) != event.event_sequence):
        raise NfeCorrectionError("A consulta retornou uma sequencia de carta de correção diferente da esperada.")
    ...
    if require_document_key and document_key and not payload_key:
        raise NfeCorrectionError("A resposta da carta de correção não identifica a NF-e relacionada.")
    if payload_key and document_key and payload_key != document_key:
        raise NfeCorrectionError("A consulta retornou uma carta de correção vinculada a outra NF-e.")
...
def consult_cce_event(*, event: FiscalDocumentEvent) -> dict[str, Any]:
    ...
    try:
        ...
        message = build_webmania_request_exception_message(exc, default="Falha ao consultar carta de correção", scope="nfe")
        raise NfeCorrectionError(message) from exc

    try:
        payload = response.json()
    except ValueError as exc:
        raise NfeCorrectionError("Resposta inválida da API de consulta da carta de correção.") from exc
    if not isinstance(payload, dict):
        raise NfeCorrectionError("Resposta inválida da API de consulta da carta de correção.")
...
def emit_nfe_correction(*, nfe_item: NfeItem, correction_text: str, requested_by: Any | None = None, request: HttpRequest | None = None) -> FiscalDocumentEvent:
    ...
    try:
        ...
        message = "Resposta inválida ao emitir carta de correção; estado remoto incerto."
        mark_attempt_uncertain(attempt=attempt, error_message=message)
        mark_cce_event_uncertain(event=event, error_message=message)
        raise NfeCorrectionError(message) from exc
    ...
    try:
        ...
        message = f"Resposta inconsistente ao emitir carta de correção; estado remoto incerto. {exc}"
        mark_attempt_uncertain(attempt=attempt, error_message=message)
