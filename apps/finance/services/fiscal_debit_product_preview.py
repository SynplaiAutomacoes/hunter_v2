...
def create_debit_product_preview(
    *,
    ...
    if locked_basis.fiscal_hypothesis != FiscalHypothesis.DEBIT_FINE_INTEREST:
        raise ValidationError("A prévia suporta somente débito tipo 4 por multa/juros.")
    ...
    if not explicit_value_confirmation:
        raise ValidationError("Confirme que referencia, quantidade, valor unitário, total e CFOP foram definidos explicitamente.")
