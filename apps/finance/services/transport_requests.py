...
def _find_transport_source(*, workshop: Any, stock_import_id: int, requested_by: Any) -> StockImport:
    stock_import = (
        StockImport.objects.filter(workshop=workshop, pk=stock_import_id)
        .select_related("fiscal_document", "workshop")
        .prefetch_related("fiscal_items__stock_product__product")
        .first()
    )
    if stock_import is None:
        raise TransportRequestError("Não encontramos a NF-e de entrada selecionada.")
    ...
    if (
        ...
        raise TransportRequestError("A NF-e de entrada não está autorizada ou não pode originar uma Nota de Transporte.")
    if not stock_import.fiscal_items.exists():
        raise TransportRequestError("A NF-e de entrada não possui itens fiscais disponíveis para transporte.")
    return stock_import


def eligible_transport_imports(*, workshop: Any) -> QuerySet[StockImport]:
    normalized = Q(
...
def search_transport_imports(*, workshop: Any, filters: Mapping[str, Any]) -> QuerySet[StockImport]:
    queryset = eligible_transport_imports(workshop=workshop)
    ...
    if filters.get("issued_from"):
        queryset = queryset.filter(purchase_issued_at__date__gte=filters["issued_from"])
    if filters.get("issued_until"):
        queryset = queryset.filter(purchase_issued_at__date__lte=filters["issued_until"])
    ...
    return queryset.filter(pk__in=normalized.values("pk"))


def select_transport_source(*, workshop: Any, stock_import_id: int, requested_by: Any) -> TransportRequest:
    stock_import = _find_transport_source(workshop=workshop, stock_import_id=stock_import_id, requested_by=requested_by)
    existing = (
