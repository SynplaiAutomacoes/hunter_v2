...
def _status_from_event_payload(payload: dict[str, Any]) -> str:
    status = str(payload.get("status") or "").strip().lower()
    if status in {FiscalDocumentEventStatus.APPROVED, FiscalDocumentEventStatus.REPROVED, FiscalDocumentEventStatus.PROCESSING, FiscalDocumentEventStatus.SUCCEEDED, FiscalDocumentEventStatus.FAILED}:
        return status
