...
class FiscalOperationGatewayTests(SimpleTestCase):
    ...
    def test_gateway_exposes_nfe_nfse_without_standalone_top_level(self) -> None:
        ...
        self.assertEqual(
            ...
            {"nfe", "nfse", "return", "correction", "complementary", "adjustment"},
        )
        self.assertNotIn("standalone", card_values)
        self.assertEqual(
    ...
    def test_nfse_continue_opens_linkage_step(self) -> None:
        request = self.factory.post(
            "/finance/emissao/",
            {"operation": FiscalOperation.NFSE, "gateway_step": "operation"},
        )
        view = self._build_view(request)
        form = FiscalOperationGatewayForm(request.POST)
        self.assertTrue(form.is_valid(), form.errors)
        ...
        self.assertRedirects(response, f"{reverse('finance:emission_create')}?fluxo=nfse", fetch_redirect_response=False)

    def test_linkage_step_renders_two_flow_cards(self) -> None:
        request = self.factory.get("/finance/emissao/", {"fluxo": "nfse"})
        view = self._build_view(request)
        response = view.get(request)

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context_data["gateway_step"], "linkage")
        self.assertEqual(response.context_data["selected_document"], "nfse")
        self.assertEqual(len(response.context_data["linkage_cards"]), 2)

    def test_nfse_standalone_redirects_to_avulsa_wizard(self) -> None:
        request = self.factory.post(
            "/finance/emissao/?fluxo=nfse",
            {
                "operation": FiscalOperation.NFSE,
