...
class PurchaseReturnWorkflowView(PurchaseReturnPermissionMixin, View):
    ...
    def post(self, request: HttpRequest, pk: int, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        if step == 2:
            ...
            try:
                return_request = save_purchase_return_items(request=return_request, quantities=form.quantities(), manual_items=form.manual_items())
            except PurchaseReturnError as exc:
    ...
    def _render(
        ...
        *,
        ...
        selected_items = list(return_request.items.select_related("source_item").order_by("kind", "source_item__sequence", "pk"))
        selected_stock_items = [item for item in selected_items if item.source_item_id is not None]
        selected_manual_items = [item for item in selected_items if item.source_item_id is None]
