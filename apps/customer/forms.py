        ...
        self.helper.layout = Layout(
            ...
            (() => {
                ...
                const api = {
                    ...
                    async loadModels(container, { preserveModel = '', preserveFuel = '', preserveEngine = '' } = {}) {
                        ...
                        if (!brand) {
                            ...
                            this.setFieldValue(modelInput, '');
                            this.setFieldValue(fuelInput, '');
                            this.syncEngine(container, preserveEngine);
                            return;
                        }
                        ...
                        this.setSearchableOptions(modelInput, options);
                        this.setFieldValue(modelInput, preserveModel);
                        await this.loadFuels(container, { preserveFuel });
                        this.syncEngine(container, preserveEngine);
                    },
                    async loadFuels(container, { preserveFuel = '' } = {}) {
                        const brand = this.getFieldValue(container, 'brand');
                        ...
                        this.setSearchableOptions(fuelInput, options);
                        this.setFieldValue(fuelInput, preserveFuel);
                    },
                    syncEngine(container, fallbackEngine = '') {
                        const engineInput = this.getField(container, 'engine');
                        if (!engineInput) {
                            return;
                        }

                        const detectedEngine = this.extractEngineFromModelName(this.getFieldValue(container, 'model')) || String(fallbackEngine || '').trim();
                        this.setSearchableOptions(engineInput, detectedEngine ? [{ id: detectedEngine, label: detectedEngine }] : []);
                        this.setFieldValue(engineInput, detectedEngine);
                    },
                    ...
                    async fillFromPlate(container, data) {
                        ...
                        try {
                            ...
                            this.setSearchableOptions(brandInput, data.brand ? [{ id: data.brand, label: data.brand }] : []);
                            this.setFieldValue(brandInput, data.brand || '');
                            await this.loadModels(container, {
                                preserveModel: data.model || '',
                                preserveFuel: data.fuel || '',
                                preserveEngine: data.engine || '',
                            });

                            const modelInput = this.getField(container, 'model');
                            const fuelInput = this.getField(container, 'fuel');
                            if (modelInput && data.model) {
                                this.setSearchableOptions(modelInput, [{ id: data.model, label: data.model }]);
                                this.setFieldValue(modelInput, data.model);
                            }
                            if (fuelInput && data.fuel) {
                                this.setSearchableOptions(fuelInput, [{ id: data.fuel, label: data.fuel }]);
                                this.setFieldValue(fuelInput, data.fuel);
                            }
