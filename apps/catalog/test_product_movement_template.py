...
class ProductMovementTemplateTests(SimpleTestCase):
    def test_renders_stock_adjustment_labels_and_empty_nf(self) -> None:
        movement = SimpleNamespace(
            criado_em=timezone.make_aware(datetime(2026, 7, 9, 0, 42)), type="ENTRADA",
            transcation_by=None, quantity=2, status="APROVADO", get_status_display="Aprovado",
            display_date=timezone.make_aware(datetime(2026, 7, 9, 0, 42)), workorder_reference=None,
            workorder_id=None, workorder=None, is_stock_adjustment=True, historical_supplier=None,
