...
class WorkOrderDetailNavigationTests(SimpleTestCase):
    ...
    def test_resume_matches_budget_step4_layout(self) -> None:
        ...
        self.assertIn("grid grid-cols-1 xl:grid-cols-12 gap-4", resume)
        self.assertIn("xl:col-span-8", resume)
        self.assertIn("xl:col-span-4", resume)
        self.assertIn('id="workorder-resume-summary"', resume)
        self.assertIn("sticky top-4", resume)
        self.assertIn(">Resumo</h2>", resume)
        self.assertNotIn("max-w-xl ml-auto", resume)
    ...
    def test_delivery_includes_emission_cta(self) -> None:
        ...
        self.assertIn("Atualizar Status da Ordem de Serviço", delivery)
        self.assertIn("Observações", delivery)
        self.assertIn("flex flex-row flex-wrap items-stretch gap-3", delivery)
        self.assertNotIn(">Aprovação</h4>", delivery)
        self.assertNotIn("grid grid-cols-12 gap-3", delivery)
        anexos_idx = delivery.index(">Anexos</h4>")
        pdf_idx = delivery.index(">PDF</h4>")
        obs_idx = delivery.index(">Observações</h4>")
        status_idx = delivery.index("Atualizar Status da Ordem de Serviço")
        self.assertLess(anexos_idx, pdf_idx)
        self.assertLess(pdf_idx, obs_idx)
        self.assertLess(obs_idx, status_idx)
