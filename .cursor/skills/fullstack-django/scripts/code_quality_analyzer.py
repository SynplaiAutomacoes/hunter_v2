...
# Skip patterns
SKIP_DIRS = {"node_modules", "vendor", ".git", "__pycache__", "dist", "build", ".next", ".venv", "venv", "env", "coverage", ".pytest_cache"}

# Security patterns to detect
SECURITY_PATTERNS = {
    "hardcoded_secret": {"pattern": r"(?:password|secret|api_key|apikey|token|auth)[\s]*[=:][\s]*['\"][^'\"]{8,}['\"]", "severity": "critical", "message": "Potential hardcoded secret detected"},
    "sql_injection": {"pattern": r"(?:execute|query|raw)\s*\(\s*[f'\"].*\{.*\}|%s|%d|\$\d", "severity": "high", "message": "Potential SQL injection vulnerability"},
    "xss_vulnerable": {"pattern": r"innerHTML\s*=|v-html", "severity": "medium", "message": "Potential XSS vulnerability - unescaped HTML rendering"},
    "unsafe_react_html": {"pattern": r"__html", "severity": "medium", "message": "React unsafe HTML pattern detected - ensure content is sanitized"},
    "insecure_protocol": {"pattern": r"http://(?!localhost|127\.0\.0\.1)", "severity": "medium", "message": "Insecure HTTP protocol used"},
    "debug_code": {"pattern": r"console\.log|print\(|debugger|DEBUG\s*=\s*True", "severity": "low", "message": "Debug code should be removed in production"},
    "todo_fixme": {"pattern": r"(?:TODO|FIXME|HACK|XXX):", "severity": "info", "message": "Unresolved TODO/FIXME comment"},
}
...
CODE_SMELL_PATTERNS = {
    ...
    "magic_number": {"pattern": r"(?<![a-zA-Z_])\b(?:[2-9]\d{2,}|\d{4,})\b(?![a-zA-Z_])", "description": "Magic number should be named constant"},
}
...
def read_text(filepath: Path) -> str:
    """Read file content with a forgiving UTF-8 fallback."""
...
def calculate_complexity(content: str, language: str) -> JsonDict:
    """Calculate cyclomatic complexity estimate."""
    # Count decision points
    decision_patterns = [r"\bif\b", r"\belse\b", r"\belif\b", r"\bfor\b", r"\bwhile\b", r"\bcase\b", r"\bcatch\b", r"\b\?\b", r"\b&&\b", r"\b\|\|\b", r"\band\b", r"\bor\b"]
    ...
    return {"cyclomatic": complexity, "max_nesting": max_depth, "rating": "low" if complexity < 10 else "medium" if complexity < 20 else "high"}


def analyze_security(filepath: Path, content: str) -> list[JsonDict]:
    ...
    for pattern_name, pattern_info in SECURITY_PATTERNS.items():
        ...
        for line_num, line in enumerate(lines, 1):
            if regex.search(line):
                issues.append({"file": str(filepath), "line": line_num, "type": pattern_name, "severity": pattern_info["severity"], "message": pattern_info["message"]})

    return issues


def analyze_dependencies(project_path: Path) -> JsonDict:
    """Analyze project dependencies for issues."""
    findings = {"package_managers": [], "total_deps": 0, "outdated": [], "vulnerable": [], "recommendations": []}
            ...
            for dep, version in deps.items():
                ...
                if dep in KNOWN_VULNERABLE_DEPS:
                    ...
                    if clean_version and clean_version < vuln["vulnerable_below"]:
                        findings["vulnerable"].append({"package": dep, "current": version, "fix_version": vuln["vulnerable_below"], "cve": vuln["cve"]})
    ...
    if requirements.exists():
        ...
        try:
            lines = [line.strip() for line in read_text(requirements).splitlines() if line.strip() and not line.startswith("#")]
...
def analyze_test_coverage(project_path: Path) -> JsonDict:
    ...
    return {"source_files": source_count, "test_files": test_count, "estimated_coverage": ratio, "rating": "good" if ratio >= 70 else "adequate" if ratio >= 40 else "poor", "recommendation": None if ratio >= 70 else f"Consider adding more tests ({70 - ratio}% gap to target)"}


def analyze_documentation(project_path: Path) -> JsonDict:
    """Analyze documentation quality."""
    docs = {"has_readme": False, "has_contributing": False, "has_license": False, "has_changelog": False, "api_docs": [], "score": 0}
...
def analyze_project(project_path: Path) -> JsonDict:
    ...
    for filepath in project_path.rglob("*"):
        ...
        # Track by language
        lang = "typescript" if filepath.suffix in {".ts", ".tsx"} else "javascript" if filepath.suffix in {".js", ".jsx"} else "python" if filepath.suffix == ".py" else "go" if filepath.suffix == ".go" else "other"
        ...
        if complexity["rating"] == "high":
            results["complexity"]["high_complexity_files"].append({"file": str(filepath.relative_to(project_path)), "complexity": complexity["cyclomatic"], "nesting": complexity["max_nesting"]})
        ...
        if total > CODE_SMELL_PATTERNS["large_file"]["threshold"]:
            results["code_smells"].append({"file": str(filepath.relative_to(project_path)), "type": "large_file", "details": f"{total} lines (threshold: {CODE_SMELL_PATTERNS['large_file']['threshold']})"})
    ...
    results["grade"] = "A" if score >= 90 else "B" if score >= 80 else "C" if score >= 70 else "D" if score >= 60 else "F"
...
def generate_recommendations(analysis: JsonDict) -> list[JsonDict]:
    ...
    for issue in analysis["security"]["critical"][:3]:
        recs.append({"priority": "P0", "category": "security", "issue": issue["message"], "file": issue["file"], "action": f"Remove or secure sensitive data at line {issue['line']}"})

    # Vulnerable dependencies
    for vuln in analysis["dependencies"].get("vulnerable", [])[:3]:
        recs.append({"priority": "P0", "category": "security", "issue": f"Vulnerable dependency: {vuln['package']} ({vuln['cve']})", "action": f"Update to version {vuln['fix_version']} or later"})

    # High security issues
    for issue in analysis["security"]["high"][:3]:
        recs.append({"priority": "P1", "category": "security", "issue": issue["message"], "file": issue["file"], "action": "Review and fix security vulnerability"})

    # Test coverage
    tests = analysis.get("tests", {})
    if tests.get("estimated_coverage", 0) < 50:
        recs.append({"priority": "P1", "category": "quality", "issue": f"Low test coverage: {tests.get('estimated_coverage', 0)}%", "action": "Add unit tests to improve coverage to at least 70%"})

    # High complexity files
    for cplx in analysis["complexity"]["high_complexity_files"][:2]:
        recs.append({"priority": "P2", "category": "maintainability", "issue": f"High complexity in {cplx['file']}", "action": "Refactor to reduce cyclomatic complexity"})

    # Documentation
    docs = analysis.get("documentation", {})
    if not docs.get("has_readme"):
        recs.append({"priority": "P2", "category": "documentation", "issue": "Missing README.md", "action": "Add README with project overview and setup instructions"})

    return recs[:10]


def print_report(analysis: JsonDict, verbose: bool = False) -> None:
...
Examples:
  ...
  %(prog)s /path/to/project --json --output report.json
    ...
    parser.add_argument("project_path", nargs="?", default=".", help="Path to project directory (default: current directory)")
