...
TEMPLATES = {
    "nextjs": {
        ...
        "structure": {
            ...
            "src/types": ["index.ts"],
            "public": [],
            "": ["package.json", "tsconfig.json", "tailwind.config.ts", "next.config.js", ".env.example", ".gitignore", "README.md"],
        },
    },
    "fastapi-react": {
        "name": "FastAPI + React",
...
def create_project(template_name: str, project_name: str, output_dir: Path) -> JsonDict:
    ...
    for dir_path, files in template["structure"].items():
        ...
        for filename in files:
            ...
            relative_path = str(Path(dir_path) / filename) if dir_path else filename
            content = get_file_content(template_name, relative_path, project_name)
            filepath.write_text(content, encoding="utf-8")
            created_files.append(str(filepath))

    return {"success": True, "project_name": project_name, "template": template_name, "description": template["description"], "location": str(project_dir), "files_created": len(created_files), "directories_created": len(created_dirs), "next_steps": get_next_steps(template_name, project_name)}
...
def get_next_steps(template: str, name: str) -> list[str]:
    ...
    steps = {
        ...
        "fastapi-react": [f"cd {name}", "docker-compose up -d db", "cd backend && pip install -r requirements.txt && uvicorn app.main:app --reload", "cd frontend && npm install && npm run dev"],
        "mern": [f"cd {name}", "docker-compose up -d mongo", "cd server && npm install && npm run dev", "cd client && npm install && npm run dev"],
        "django-react": [f"cd {name}", "docker-compose up -d db", "cd backend && pip install -r requirements.txt && python manage.py migrate && python manage.py runserver", "cd frontend && npm install && npm run dev"],
    }
    return steps.get(template, [f"cd {name}"])


def list_templates() -> JsonDict:
    """List available templates."""
    return {"templates": [{"name": k, "display_name": v["name"], "description": v["description"]} for k, v in TEMPLATES.items()]}


def print_result(result: JsonDict, as_json: bool = False) -> None:
    """Print result."""
    if as_json:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate fullstack project scaffolding", epilog="Examples:\n  %(prog)s nextjs my-app\n  %(prog)s fastapi-react my-api\n  %(prog)s --list-templates", formatter_class=argparse.RawDescriptionHelpFormatter)
